...
import 'package:demo09/api/config/http_exceptions.dart';
    ...
    switch (error.type) {
      ...
      case DioErrorType.response:
        try {
          ...
          String errorMsg(msg) => '${error.response?.data['msg'] ?? msg}';
          switch (errCode) {
            case 400:
              return BadRequestException(
                  message: error.response?.data['msg'] ?? "请求语法错误",
                  code: errCode);
            case 401:
              return UnauthorisedException(
                  message: errorMsg("没有权限"), code: errCode);
            case 403:
              return BadRequestException(
                  message: errorMsg("服务器拒绝执行"), code: errCode);
            case 404:
              return BadRequestException(
                  message: errorMsg("无法连接服务器"), code: errCode);
            case 405:
              return BadRequestException(
                  message: errorMsg("请求方法被禁止"), code: errCode);
            case 500:
              return BadServiceException(
                  message: errorMsg("服务器内部错误"), code: errCode);
            case 502:
              return BadServiceException(
                  message: errorMsg("无效的请求"), code: errCode);
            case 503:
              return BadServiceException(
                  message: errorMsg("服务器挂了"), code: errCode);
            case 505:
              return UnauthorisedException(
                  message: errorMsg("不支持HTTP协议请求"), code: errCode);
            default:
